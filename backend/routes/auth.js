...
const registerRules = [
  body('username').trim().isLength({ min: 3, max: 30 }).withMessage('Kullanıcı adı 3–30 karakter olmalıdır.').matches(/^[a-zA-Z0-9_]+$/).withMessage('Kullanıcı adı yalnızca harf, rakam ve alt çizgi içerebilir.'),
  body('email').trim().isEmail().withMessage('Geçerli bir e-posta adresi girin.').normalizeEmail(),
  body('password').isLength({ min: 8 }).withMessage('Şifre en az 8 karakter olmalıdır.').matches(/[A-Z]/).withMessage('Şifre en az bir büyük harf içermelidir.').matches(/[0-9]/).withMessage('Şifre en az bir rakam içermelidir.'),
];
...
router.post('/register', registerRules, validate, async (req, res, next) => {
  try {
    ...
    return res.status(201).json({ success: true, message: 'Kayıt başarılı. Hoş geldiniz!', data: { token, user: User.sanitize(user) } });
  } catch (err) {
    next(err);
  }
});
...
router.post('/login', loginRules, validate, async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ success: true, message: 'Giriş başarılı.', data: { token, user: User.sanitize(user) } });
  } catch (err) {
    next(err);
  }
});
...
router.post('/logout', requireAuth, (req, res) => {
  return res.json({ success: true, message: 'Oturum başarıyla kapatıldı.' });
});
